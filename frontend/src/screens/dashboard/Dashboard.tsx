...
export default function Dashboard() {
  ...
  const displayAvatar = user?.image || '/assets/profile.jpg';
    ...
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-purple-900 text-white">
      ...
      <header className="px-6 py-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <img src={displayAvatar} alt={displayName} className="w-12 h-12 rounded-full border-2 border-pink-400 shadow" />
      ...
      <section className="max-w-4xl mx-auto mb-10 px-4">
        ...
        <div className="flex flex-wrap gap-6 justify-center">
          {trendingCreators.map((creator, idx) => (
            <div key={idx} className="bg-white/10 border border-white/20 rounded-xl px-6 py-4 shadow-lg flex flex-col items-center min-w-[140px] hover:bg-pink-500/20 transition">
              <img src={creator.avatar} alt={creator.name} className="w-16 h-16 rounded-full border-2 border-pink-400 shadow mb-2" />
              <span className="font-bold text-lg">{creator.name}</span>
              <span className="text-xs text-gray-300">{creator.followers.toLocaleString()} followers</span>
              <button className="mt-2 px-4 py-1 rounded-full bg-pink-500 text-white text-xs font-semibold hover:bg-pink-600 transition">Follow</button>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-4xl mx-auto mb-10 px-4">
        ...
        <div className="rounded-2xl overflow-hidden shadow-2xl border border-white/10">
          <button onClick={() => navigate('/feed')} className="w-full py-4 bg-gradient-to-r from-pink-500 via-red-500 to-yellow-500 hover:from-pink-600 hover:to-yellow-600 text-white font-bold text-lg transition">
            Explore More Reels
          </button>
        </div>
      </section>
